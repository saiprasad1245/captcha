...
import com.captcha.config.RazorPayClientConfig;
import com.captcha.dto.ApiResponse;
import com.captcha.dto.LocalUser;
import com.captcha.dto.payment.OrderRequest;
import com.captcha.dto.payment.OrderResponse;
import com.captcha.dto.payment.PaymentResponse;
import com.captcha.service.OrderService;
...
public class OrderController {
	...
	public ResponseEntity<?> createOrder(@RequestBody OrderRequest orderRequest, @CurrentUser LocalUser user) {
		...
		try {
			...
			orderService.saveOrder(razorPay.getRazorpayOrderId(), (long) 1);
	...
	public ResponseEntity<?> updateOrder(@RequestBody PaymentResponse paymentResponse) {
		String errorMsg = orderService.validateAndUpdateOrder(paymentResponse.getRazorpayOrderId(), paymentResponse.getRazorpayPaymentId(), paymentResponse.getRazorpaySignature(),
				razorPayClientConfig.getSecret(),paymentResponse.getUserName(),paymentResponse.getAmount(),paymentResponse.getType());
		if (errorMsg != null) {
